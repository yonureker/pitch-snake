...
19. Going through pays `PORTAL_BONUS`, and pays it **once per pair** (`portal.scored`), never per hop. Both ends work both ways, so a per-hop award is a four step loop farmed for points. The award sits after the wall, self, ghost and TNT tests in `step()`, so a hop that kills you pays nothing; nothing else can share a landing cell, so it is the only score change that tick.
