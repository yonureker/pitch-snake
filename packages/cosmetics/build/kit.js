...
export const KIT_NONE = { left: null, right: null, num: null };
/**
 ...
 */
export function kitColor(value) {
 ...
 */
export function kitNumber(value) {
    let n = value;
    if (typeof n === 'string') {
        const text = n.trim();
 ...
 */
export function kitNumbersFor(chosen, dealt) {
    const taken = new Set();
    const out = [];
    ...
    for (let lane = 0; lane < chosen.length; lane++) {
        if (out[lane] !== undefined)
            continue;
        const fallback = dealt[lane] ?? KIT_NUM_MIN;
        let pick = fallback;
        if (taken.has(pick)) {
            pick = KIT_NUM_MIN;
            while (taken.has(pick) && pick < KIT_NUM_MAX)
                pick++;
        }
        taken.add(pick);
        out[lane] = pick;
