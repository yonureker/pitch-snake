...
function armedRoom(seed = 7) {
  ...
  g.portalRetryAt = 1e12;
  foodFar(g);
  setPlayerSnake(g, 0, [[5, 10], [4, 10], [3, 10]], 1, 0);
  setPlayerSnake(g, 1, [[14, 10], [13, 10], [12, 10]], 1, 0);
  return g;
}

test('a sight falls due every eight pickups, counting everything picked up', () => {
  assert.equal(SIGHT_EVERY, 8);
  const g = armedRoom();
  g.pickups = 7;
  g._updateSight();
  assert.equal(g.sight, null, 'seven is not eight');
  g.pickups = 8;
  g._updateSight();
  assert.ok(g.sight, 'the eighth arms the board');
  // a trip and a bolt count toward it as much as a mouthful does
  const h = armedRoom(9);
  h.portal = { ax: 5, ay: 10, bx: 15, by: 15, used: false };
  h._stepPlayer(0);                              // player 0 walks into the window
  assert.equal(h.pickups, 1, 'a teleport trip counts');
  h.bolt = { x: h.players[1].snake[0].x + 1, y: 10, bornAt: 0 };
  h._stepPlayer(1);
  assert.equal(h.pickups, 2, 'and so does a bolt');
});

test('a room only: one snake on the board never sees a sight', () => {
  const solo = quietGame();
  solo.pickups = 80;
  solo._updateSight();
  assert.equal(solo.sight, null, 'nothing to shoot, nothing to shoot with');
});

test('taking a sight arms five shots, the first a second later, one a second', () => {
  const g = armedRoom();
  g.sight = { x: 6, y: 10, bornAt: 0 };
  g._stepPlayer(0);
  assert.equal(g.players[0].shotsLeft, SHOT_COUNT);
  assert.equal(g.players[0].nextShotAt, g.clockMs + SHOT_EVERY_MS, 'the first waits a second');
  assert.equal(g.sight, null, 'taken');
  assert.equal(g.score, 0, 'a sight is not food: it scores nothing');
  const fired = [];
  for (let q = 0; q < 700; q++) {
    const before = g.shots.length;
    g.advanceQuanta(1);
    for (const e of g.drainEvents()) if (e.t === 'shot') fired.push(g.clockMs);
    if (g.shots.length > before) { /* one left the barrel */ }
  }
  assert.equal(fired.length, SHOT_COUNT, 'five shots and no more');
  for (let i = 1; i < fired.length; i++) {
    assert.equal(fired[i] - fired[i - 1], SHOT_EVERY_MS, 'a second apart');
  }
});

test('a shot down the lane kills a rival head, and stops there', () => {
  const g = armedRoom();
  // facing the shooter: chased from behind, a snake's own tail takes the shot
  setPlayerSnake(g, 1, [[14, 10], [15, 10], [16, 10]], -1, 0);
...
test('a shot stops on a body without killing it', () => {
  const g = armedRoom();
  // The rival lies across the lane with its head safely off it, and is held
  // still so the test is about the body and not about who moved where: a
  // snake on its own clock that never reaches it simply never steps.
  setPlayerSnake(g, 1, [[14, 8], [14, 9], [14, 10]], 0, -1);
...
test('two rival heads on one square is one shot and two deaths', () => {
  const g = quietGame({ players: 3 });
  g.portalRetryAt = 1e12;
  foodFar(g);
  setPlayerSnake(g, 0, [[5, 10], [4, 10], [3, 10]], 1, 0);
...
test('a round with a shooting kill replays exactly, and rolls back the same', () => {
  const mk = () => {
    const g = armedRoom(21);
    g.sight = { x: 6, y: 10, bornAt: 0 };
    g._stepPlayer(0);
    return g;
  };
