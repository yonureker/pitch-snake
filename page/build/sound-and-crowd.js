...
function audioResume() {
    if (audioResuming || rig === null || rig.ctx.state === 'running')
        return;
    audioResuming = true;
    const done = () => {
        audioResuming = false;
...
function audioHook() {
    ...
    for (const ev of WAKE_EVENTS)
        window.addEventListener(ev, audioWake, { passive: true });
}
function audioUnhook() {
    ...
    wakeHooked = false;
    for (const ev of WAKE_EVENTS)
        window.removeEventListener(ev, audioWake);
}
audioHook();
document.addEventListener('visibilitychange', () => {
    if (!document.hidden && pageSound)
        audioResume();
});
/**
...
function sfxTone(r, type, f0, f1, at, dur, peak) {
...
function sfxBoom(r, at, dur, peak, f0, f1) {
...
function crowdVoice(r, buf, rate, type, freq, q, level, dest) {
    const src = r.ctx.createBufferSource();
    src.buffer = buf;
    src.loop = true;
...
function crowdFetch(r, path, then) {
 ...
 */
export function crowdVerdict(won) {
    const r = liveRig();
    if (!crowdOn || r === null)
        return;
    const t = r.ctx.currentTime;
...
function crowdArm() {
    if (!crowdSurgeTimer)
        crowdSurgeTimer = window.setTimeout(crowdSurge, 12000 + Math.random() * 20000);
}
function crowdSurge() {
    crowdSurgeTimer = 0;
    const r = crowdRig();
...
function crowdRig() {
    if (!crowdOn)
        return null;
    if (roundPhase !== 'playing' && roundPhase !== 'countdown')
        return null;
    return liveRig();
}
/**
 ...
 */
export function crowdSync(phase) {
    if (phase !== undefined)
        roundPhase = phase;
 ...
 */
export function crowdPulse(k) {
    const r = liveRig();
    const cheer = crowdCheer;
    if (cheer === null || !crowdOn || r === null)
        return;
    const t = r.ctx.currentTime;
...
export function isPageSoundOn() {
    return pageSound;
}
/** Whether the crowd is allowed to be heard. */
export function isCrowdOn() {
    return crowdOn;
}
export { audioHook, audioWake };
//# sourceMappingURL=sound-and-crowd.js.map