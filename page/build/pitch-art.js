...
export const HATS = {
    classic: {
        wf: 1.5, hf: 0.95,
        dy: (cellPixels, height) => -cellPixels * 0.42 - height * 0.62,
        draw(c, w, h) {
            const midX = w / 2;
    ...
    'hat-cap': {
        wf: 1.35, hf: 0.62,
        dy: (cellPixels, height) => -cellPixels * 0.38 - height * 0.58,
        draw(c, w, h) {
    ...
    'hat-crown': {
        wf: 1.1, hf: 0.72,
        dy: (cellPixels, height) => -cellPixels * 0.4 - height * 0.52,
        draw(c, w, h) {
...
const SKIN_BY_ID = SKINS;
const HAT_BY_ID = HATS;
/**
 ...
 */
export function buildLutFor(skin) {
 ...
 */
export function roundRectOn(c, x, y, w, h, rad) {
    c.beginPath();
    c.moveTo(x + rad, y);
 ...
 */
export function drawCrest(c, w, h, skin) {
    if (skin.spikes === null)
        return;
 ...
 */
export function paintJersey(c, s, num) {
    c.clearRect(0, 0, s, s);
    c.save();
    ...
    const ink = (color, dx, dy) => {
        ...
        for (let d = 0; d < digits.length; d++) {
            ...
            for (let r = 0; r < 5; r++)
                for (let k = 0; k < 3; k++)
                    if (glyph[r]?.[k] === '1')
                        c.fillRect(x0 + d * px * 4 + k * px + dx, y0 + r * px + dy, px, px);
        }
    };
    ink('rgba(33,30,26,0.8)', 1, 1); // the drop shadow keeps white legible on yellow
    ink('#ffffff', 0, 0);
}
//# sourceMappingURL=pitch-art.js.map