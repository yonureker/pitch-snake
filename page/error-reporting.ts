...
/** The slice of the Sentry browser SDK this module actually uses. */
 ...
 */
function signatureOf(event: unknown): string {
  const values = read(read(event, 'exception'), 'values');
  const head = Array.isArray(values) ? (values[0] as unknown) : undefined;
...
function attach(): void {
  try {
    ...
    loaded.init({
      ...
      beforeSend: (event: unknown): unknown => (allow(signatureOf(event)) ? event : null),
    });
    sentry = loaded;
  } catch {
    /* reporting stays off */
...
export function initErrorReporting(): void {
  if (!reporting.dsn) return; // off, deliberately and completely
  try {
 ...
 */
export function reportIssue(kind: string, detail?: Record<string, string | number | boolean>): void {
  try {
